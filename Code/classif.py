...
def appel_process_image(image_path, output_folder, image_type):
    ...
    elif image_type=='tested' :
        return convert_arrays_to_tuples(process_image_sable(image_path, output_folder, n_segments=15000, compactness=10, window_size=3))
    else:
...
def kmeans(image_path, output_folder, n_clusters=2, n_init=15, init='k-means++', max_iter=300, tol=1e-4, min_contour_length=1000):
    ...
    for contour in valid_contours:
        # Dessiner les contours en rouge (BGR: (0, 0, 255) = rouge)
        cv2.drawContours(image, [contour], -1, (0, 0, 255), 2)  # 2 est l'épaisseur du contour
