...
from classif import enregistrer_points
...
def apply_homography(pts_target, pts_query, query_img_path, target_img_path, output_path):
    ...
    # Dessiner les lignes rouges et points verts
    for i in range(1, len(points_transformed)):
        pt1 = tuple(map(int, points_transformed[i-1]))
        pt2 = tuple(map(int, points_transformed[i]))
    ...
    gdf = gpd.GeoDataFrame(geometry=[Point(x, y) for x, y in geo_points], crs=projection)
