...
def asift_main(image1: str, image2: str, detector_name: str = "sift-flann",output_folder:str="output"):
    """
    ...
    matched_points = [(kp1.pt, kp2.pt) for kp1, kp2 in kp_pairs]

    # Afficher les 5 premiers points
    print(matched_points[:5])
    # Convertir la liste des points en un DataFrame avec les bonnes colonnes
    df = pd.DataFrame(matched_points, columns=["(x1, y1)", "(x2, y2)"])

    # Séparer les tuples en colonnes x1, y1, x2, y2
    df[["x1", "y1"]] = pd.DataFrame(df["(x1, y1)"].tolist(), index=df.index)
    df[["x2", "y2"]] = pd.DataFrame(df["(x2, y2)"].tolist(), index=df.index)
    ...
    print(f"Found {len(kp_pairs)} keypoint pairs")

    matched_img =draw_match("ASIFT Match Result", ori_img1, ori_img2, kp_pairs, None, H)     # Visualize result
    #cv2.waitKey()

    output_image_name="matching.png"
    output_image_path = os.path.join(output_folder, output_image_name)
    cv2.imwrite(output_image_path, matched_img)  

    output_keypoint = os.path.join(output_folder, "keypoints.txt")
    log_keypoints(kp_pairs, output_keypoint)     # Save keypoint pairs for further inspection
...
if __name__ == '__main__':
    ...
    asift_main("crop.png", "image_hd.jpg", "sift-flann")
