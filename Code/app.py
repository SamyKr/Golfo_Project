...
def process_image():
    ...
    # Supprimer le contenu du dossier de sortie s'il existe déjà
    if os.path.exists(output_folder):
        supprimer_contenu_dossier(output_folder)
        logs.append(f"🚮 Contenu du dossier de sortie supprimé : {output_folder}")
        socketio.emit('log', {'message': f"🚮 Contenu du dossier de sortie supprimé : {output_folder}"})
...
if __name__ == '__main__':
    ...
    time.sleep(1)
    webbrowser.open("http://localhost:5000")  # Remplace par ton URL si nécessaire
